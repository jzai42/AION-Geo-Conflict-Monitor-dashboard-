...
function hashSnapshotPayload(data: DashboardData): string {
  const payload = {
    date: data.date,
...
export function getPdfCacheKey(data: DashboardData, lang: PdfLang): string {
  return `${data.date}|${lang}|${data.version}|${hashSnapshotPayload(data)}|pdfT${PDF_TEMPLATE_REVISION}`;
}
...
export function buildPdfSnapshot(
  ...
  return {
    ...
    keyChange: data.keyChange,
    investmentSignal: data.investmentSignal,
    contentHash: hashSnapshotPayload(data),
  };
}
