...
The shipped static dashboard prioritizes **shareable URLs** (copy link / system share) and the core monitoring UI. Click **Share → Generate today's snapshot** to render a client-side PNG of the current day's report (no PDF API required), then share the image, download it, or copy the snapshot page link.
